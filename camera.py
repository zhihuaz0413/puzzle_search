...
class Cam(object):
    ...
    def __init__(self, thresholds, gain = 5):
        """
        ...
        self.h_fov = 70.8
        self.v_fov = 55.6
        self.camera_elevation_angle = 10.2 ## -11
 ...
 ## changed get blobs bottom to take threshold_idx so it only gets blobs of a certain colour
    def get_blobs_bottom(self,threshold_idx) -> tuple:
        """
        ...
        blobs = img.find_blobs([self.thresholds[threshold_idx]],pixels_threshold=150,area_threshold=150,
    ...
    def find_blobs(self, blobs, threshold_idx: int):
        colours = self.get_blob_colours(blobs)
        found_idxs = []

        for found_idx, colour in enumerate(colours):
            if colour == pow(2, threshold_idx):
                found_idxs.append(found_idx)
...
if __name__ == "__main__":
    ...
    thresholds = [
                  (34, 52, -10, 11, -56, -16), # blue
                  (21, 33, 24, 49, 11, 45), # red
    ...
    # Try uncommenting these lines to see what happens
            ...
            print(f"Distance to object: {round(0.3411*math.exp(0.0188* (sensor.height()-(blob.cy() + blob.h()/2))/y),2)}CM")
#     Try uncommenting these lines to see what happens
